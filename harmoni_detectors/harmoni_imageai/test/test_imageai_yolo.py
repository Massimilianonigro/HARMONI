...
class TestImageAI_Common(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.image = cv2.imread(rospy.get_param("test_imageai_yolo_input"))
        ...
        rospy.loginfo(
            f"Testside-expected detection: {DetectorNameSpace.imageai_yolo.value}default"
        )

        # startup imageai node
        self.server = DetectorNameSpace.imageai_yolo.name + "_" + self.instance_id
...
def main():
    ...
    rostest.rosrun(PKG, "test_imageai_yolo", TestImageAI_Valid, sys.argv)
