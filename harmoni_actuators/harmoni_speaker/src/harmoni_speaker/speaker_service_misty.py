...
class SpeakerService(HarmoniServiceManager):
        ...
        try:
            if type(data) == str:
                if ".wav" in data:
                    ...
                    #print(data)
                    outdir = self.rospack.get_path("harmoni_speaker") + "/test/test.waccc"
                    #sf.write(outdir, np.fromstring(data["audio_data"], dtype=float), data["audio_frame"])
                    data = self.file_path_to_audio_data(self.rospack.get_path("harmoni_tts") + "/temp_data/tts.mp3")
                    duration = data["duration"]
                    data = data["audio_data"]
    ...
    #Always overwrite the same file, so it's discarded the next time something is reproduced
            ...
            #print(response)
            rospy.loginfo("Waiting for audio to end " + str(duration))
            rospy.sleep(duration)  
            self.state = State.SUCCESS
            self.actuation_completed = True
            self.response_received = True
            self.result_msg = response.text
            rospy.loginfo("----------------------AUDIO REPRODUCED------------------------- RESPONSE IS " + str(self.state.value))
