...
function startFace(
    ...
    listener_eyes = new ROSLIB.Topic({
        ros: ros,
        name: '/harmoni/actuating/face/default/expressing/eyes',
        messageType: 'harmoni_face/FaceRequest'
    });
...
//Defaults now work.
function addThinkingCloud(x=200,
    y=80,
    baseSize=200,
    spacing=10,
    small_cloud_x_pos=0.5,
    ...
    //Adding as face part
    thinking_cloud_face = new facePart("thinkingCloud",thinkingCloudGroup,x_face,y_face)
...
function addBrows(color, width, height, thickness, innerSize) {
    ...
    function getIdleCoords(side) {
        var sign = side == 'left' ? 1 : -1
        return { x2: -sign * width, y2: thickness, x1: sign * width / 8, y1: -0.75 * thickness, x0: sign * width, y0: 0 }
    }
