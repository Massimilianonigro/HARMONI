...
# but I guess I am adding it here anyway...
...
class HarmoniActionServer(object):
    ...
    def accept_new_goal(self):
        with self.action_server.lock, self.lock:
            ...
            if (
                self.is_active()
                and self.current_goal.get_goal()
    ...
    def internal_goal_callback(self, goal):
        ...
        try:
            rospy.logdebug(
                "A new goal %s has been recieved by the single goal action server",
                goal.get_goal_id().id,
            )
            ...
            if (
                ...
                if self.next_goal.get_goal() and (
                    not self.current_goal.get_goal()
