...
class MistyArmsService(HarmoniServiceManager):
    """
    Arms service
    """
    ...
    def request_gesture_misty(self,data):
        left_position = data["LeftArmPosition"]
        right_position = data["RightArmPosition"]
        left_velocity = data["LeftArmVelocity"]
        right_velocity = data["RightArmVelocity"]
        units = data["Units"] if "Units" in data.keys() else "degrees" 
        #Waiting time sent in seconds
        waiting_time = float(data["WaitingTime"])
        url, payload = self.move_arms(left_position, right_position, left_velocity, right_velocity, units)
        print(url)
        print(payload)
        try:
    ...
    def move_arms(self, LeftArmPosition=0, RightArmPosition=0, LeftArmVelocity=0, RightArmVelocity=0, Units="degrees"):
        payload = {
        ...
        print("AOOOOOOOOOOOOOOOOOO Sending request to move arms with PAYLOAD " +str(payload))
...
def main():
    service_name = ActuatorNameSpace.arms.name
    instance_id = rospy.get_param("/instance_id")
    service_id = f"{service_name}_{instance_id}"
    try:
        rospy.init_node(service_name)
        s = MistyArmsService(service_name)
