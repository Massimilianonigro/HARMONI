...
#At the moment Chooses a random gesture movement between the list, (used to implement thinking behavior)
#TODO: Make it possible to go through the list of movements in sequence
class MistyArmsMovementServicePyTree(py_trees.behaviour.Behaviour):
    def __init__(self, name, movements=[[]],random=False, script_gesture=False,gesture_filename=""):
        self.name = name
        self.blackboards = []
        self.service_client_gesture_movement = None
        self.default_movements = movements
        self.movements = []
        #If script_gesture == True, it will display the gesture movements given the gestures in the script
        ...
        self.blackboard_scene.register_key("gesture_list", access=py_trees.common.Access.WRITE)
        super(MistyArmsMovementServicePyTree, self).__init__(name)
    ...
    def setup(self):
        ...
        self.service_client_gesture_movement = HarmoniActionClient(self.name)
        self.server_name = "arms_default"
        self.service_client_gesture_movement.setup_client(self.server_name, 
    ...
    def get_movements_from_gesture_list(self):
        movements_unrolled = []
        for gesture in self.blackboard_scene.gesture_list:
    ...
    def _get_movement_from_script(self):
        new_status = None
        if self.blackboard_scene.gesture == "" and self.blackboard_scene.gesture_list == []:
            new_status = py_trees.common.Status.SUCCESS
        if self.blackboard_scene.gesture not in self.gesture_file.keys() and self.blackboard_scene.gesture_list == []:
            self.logger.debug("---------------- Gesture not found in the gesture file")
            new_status = py_trees.common.Status.FAILURE
        rospy.loginfo("Gesture found in the gesture file")
        if self.blackboard_scene.gesture_list != []:
            ...
            self.movements = self.gesture_file[self.blackboard_scene.gesture]
        rospy.loginfo("OOOOOOOOOO Curr_movement is " + str(self.movements) + " and gesture is " + str(self.blackboard_scene.gesture))     
        return new_status          
        ...
        if self.send_request:
            ...
            try:
                ...
                if self.current_mov_idx < len(self.movements) - 1:
                    ...
                    self.service_client_gesture_movement.send_goal(
            ...
            new_state = self.service_client_gesture_movement.get_state()
            ...
            if new_state == GoalStatus.ACTIVE:
                ...
                self.service_client_gesture_movement.cancel_all_goals()
