...
class TestImageAIYoloPyTree(unittest.TestCase):

    def setUp(self):
        """
        Set up the client for requesting to harmoni_imageai_yolo
        """
        rospy.init_node("test_imageai_yolo", log_level=rospy.INFO)
        ...
        additional_parameters = dict([
            (DetectorNameSpace.imageai_yolo.name,False)])   
        rospy.loginfo("--------------------"+str(additional_parameters)) 
        self.imageai_yoloPyTree =  ImageAIYoloServicePytree("imageai_yoloPyTreeTest")
        self.imageai_yoloPyTree.setup(**additional_parameters)
        rospy.loginfo("Testimageai_yolo: Started up. waiting for imageai_yolo startup")
        rospy.loginfo("Testimageai_yolo: Started")

   
    
    def test_leaf_pytree_imageai_yolo(self):
        for unused_i in range(0, 3):
            self.imageai_yoloPyTree.tick_once()
            time.sleep(0.5)
            print(self.blackboardProvaIn)
        print("\n")
        return
...
def main():
    import rostest
    rospy.loginfo("test_imageai_yolo started")
    rospy.loginfo("TestImageAIYolo: sys.argv: %s" % str(sys.argv))
    rostest.rosrun(PKG, "test_imageai_yolo_pytree", TestImageAIYoloPyTree, sys.argv)
